# Roadmap X7 Cyber Security Platform

Modul dibangun bertahap. Semua bersifat **defensif**.

## Fase 1 - Fondasi
- [x] Scaffold monorepo (frontend + backend)
- [ ] Layout dashboard, sidebar terminal, tema light/dark
- [ ] Auth (JWT, 2FA, RBAC)
- [ ] Skema database PostgreSQL (users, assets, scans, vulnerabilities, reports, notifications, audit_logs, settings)

## Fase 2 - Scanner & Intelligence
- [ ] Website Scanner (Quick/Deep, queue, live progress, scheduler)
- [ ] Security Header & Cookie Analyzer
- [ ] SSL/TLS Security
- [ ] DNS & Domain Intelligence (RDAP, WHOIS, crt.sh, DoH)
- [ ] Email Security (SPF/DKIM/DMARC)
- [ ] Technology & WAF Detection

## Fase 3 - Vulnerability & Threat
- [ ] Vulnerability Intelligence (NVD, OSV, EPSS, CISA KEV)
- [ ] Version Matching
- [ ] Threat Intelligence Feed
- [ ] Risk Engine & scoring

## Fase 4 - Visualisasi & AI
- [ ] Attack Surface Visualization (D3.js / Cytoscape)
- [ ] AI Security Assistant & rekomendasi
- [ ] Report Generator (PDF/CSV/JSON/ZIP)
- [ ] Real-time WebSocket streaming

## Halaman
Dashboard, Scanner, Assets, Monitoring, Domain Intelligence, SSL Checker, DNS Tools, Vulnerability Center, Threat Feed, Reports, Notifications, Settings.
